...
import { PurchaseStatus } from "@/app/types/purchases";

export default function PurchaseStatusesPanel() {
  const [localStatuses, setLocalStatuses] = useState<PurchaseStatus[]>([]);
  ...
  useEffect(() => {
    fetch("/api/purchase-statuses")
      .then((r) => r.json())
      .then((data) => { if (Array.isArray(data)) setLocalStatuses(data); })
      .catch(() => {});
  }, []);
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch("/api/purchase-statuses", {
      method: "POST",
  ...
  const handleDelete = async (id: number) => {
    const res = await fetch(`/api/purchase-statuses/${id}`, { method: "DELETE" });
  ...
  const handleEdit = (status: PurchaseStatus) => {
  ...
  const handleSave = async (id: number) => {
    ...
    const res = await fetch(`/api/purchase-statuses/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: editingName,
